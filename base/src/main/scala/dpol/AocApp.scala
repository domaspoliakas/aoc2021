...
abstract class AocAppStreamed(day: Int, part: DoPart = DoPart.Both) extends IOApp.Simple:
  ...
  override def run: IO[Unit] =
    IO.whenA(part == DoPart.One || part == DoPart.Both)(
      IO.println("Part 1:") >>
        part1(toStream(Path(show"/home/dpol/misc/aoc2021/$dayF/input${if (!diffInput) ""
        else "1"}.txt"))).flatMap(v => IO.println(show"$v"))
    ) >>
      IO.whenA(part == DoPart.Two || part == DoPart.Both)(
        IO.println("Part 2:") >>
